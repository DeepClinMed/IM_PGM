...
# CUDA_VISIBLE_DEVICES=0 python main_mtl_concat_DFS.py --drop_out --early_stopping --lr 1e-4 --k_start 2 --k_end 3 --exp_code DFS_test  --task DFS  --log_data  --data_root_dir /GPUFS/sysu_yfyu_1/data/tcga/split_result/features/pt_files --split_dir DFS --max_epochs 4 --patient_start True
...
if args.task == 'dummy_mtl_concat':
    ...
    dataset = Generic_MIL_MTL_Dataset(csv_path = 'dataset_csv/dummy_dataset.csv',
                            ...
                            patient_strat= False)
elif args.task == 'patient2slide_concat_22cell':
    patient_path = './dataset_csv/patient_dataset_22cell.csv'
    df_patient = pd.read_csv(patient_path, index_col=0)
    label_cols = list(df_patient.columns[2:])
    args.n_classes = 2
    dataset = Patient_22CELL_MTL_Dataset(csv_path=patient_path,
                                      data_dir = os.path.join(args.data_root_dir, 'pt_files_p'),
                                      shuffle=False,
                                      seed=args.seed,
                                      print_info=True,
                                      label_dicts=[{1: 1, 0: 0} for i in range(22)],
                                      label_cols=label_cols,
                                      patient_strat=False)
elif args.task == 'patient2slide_concat_sybtype':
    patient_path = './dataset_csv/patient_dataset_sybtype.csv'
    df_patient = pd.read_csv(patient_path, index_col=0)
    label_cols = list(df_patient.columns[2:])
    args.n_classes = 4
    dataset = Generic_MIL_MTL_Dataset(csv_path=patient_path,
                                      data_dir = os.path.join(args.data_root_dir, 'pt_files_p'),
                                      shuffle=False,
                                      seed=args.seed,
                                      print_info=True,
                                      label_dicts=[{3: 3, 2: 2, 1: 1, 0: 0} for i in range(1)],
                                      label_cols=label_cols,
                                      patient_strat=False)
elif args.task == 'patient2slide_concat_sybtype_925':
    patient_path = './dataset_csv/patient_dataset_sybtype.csv'
    df_patient = pd.read_csv(patient_path, index_col=0)
    label_cols = list(df_patient.columns[2:])
    args.n_classes = 4
    dataset = Generic_MIL_MTL_Dataset(csv_path=patient_path,
                                      ...
                                      label_cols=label_cols,
                                      patient_strat=False)
elif args.task == 'patient2slide_concat_sybtype_1091':
    patient_path = './dataset_csv/patient_dataset_sybtype_1091.csv'
    df_patient = pd.read_csv(patient_path, index_col=0)
    label_cols = list(df_patient.columns[2:])
    args.n_classes = 4
    dataset = Generic_MIL_MTL_Dataset(csv_path=patient_path,
                                      ...
                                      label_cols=label_cols,
                                      patient_strat=False)
elif args.task == 'lncRNA':
    patient_path = './dataset_csv/lncRNA_subtype.csv'
    df_patient = pd.read_csv(patient_path, index_col=0)
    label_cols = list(df_patient.columns[2:])
    args.n_classes = 3
    dataset = Patient_SYBTYPE_MTL_Dataset(csv_path=patient_path,
                                      data_dir = os.path.join(args.data_root_dir, 'pt_files_p'),
                                      shuffle=False,
                                      seed=args.seed,
                                      print_info=True,
                                      label_dicts=[{3: 2, 2: 1, 1: 0} for i in range(1)],
                                      label_cols=label_cols,
                                      patient_strat=False)
elif args.task == 'AC004832-4':
    patient_path = './dataset_csv/AC004832-4_low_high.csv'
    df_patient = pd.read_csv(patient_path, index_col=0)
    label_cols = list(df_patient.columns[2:])
    args.n_classes = 2
    dataset = Patient_SYBTYPE_MTL_Dataset(csv_path=patient_path,
                                      ...
                                      seed=args.seed,
                                      print_info=True,
                                      label_dicts=[{'high': 1, 'low': 0} for i in range(1)],
                                      label_cols=label_cols,
                                      patient_strat=False)
elif args.task == 'AC008663-1':
    patient_path = './dataset_csv/AC008663_1.csv'
    df_patient = pd.read_csv(patient_path, index_col=0)
    label_cols = list(df_patient.columns[2:])
    args.n_classes = 2
    dataset = Patient_SYBTYPE_MTL_Dataset(csv_path=patient_path,
                                      ...
                                      label_cols=label_cols,
                                      patient_strat=False)
elif args.task == 'TCGA-Nstage':
    patient_path = './dataset_csv/TCGA-Nstage.csv'
    df_patient = pd.read_csv(patient_path, index_col=0)
    label_cols = list(df_patient.columns[2:])
    args.n_classes = 2
    dataset = Patient_SYBTYPE_MTL_Dataset(csv_path=patient_path,
                                      ...
                                      seed=args.seed,
                                      print_info=True,
                                      label_dicts=[{'N3': 1 ,'N2': 1 ,'N1': 1, 'N0': 0} for i in range(1)],
                                      label_cols=label_cols,
                                      patient_strat=False)
elif args.task == 'TCGA-Cluster':
    patient_path = './dataset_csv/TCGA分型.csv'
    df_patient = pd.read_csv(patient_path, index_col=0)
    label_cols = list(df_patient.columns[2:])
    args.n_classes = 4
    dataset = Patient_SYBTYPE_MTL_Dataset(csv_path=patient_path,
                                      ...
                                      seed=args.seed,
                                      print_info=True,
                                      label_dicts=[{4:3, 3:2, 2: 1, 1: 0} for i in range(1)],
                                      label_cols=label_cols,
                                      patient_strat=True)
else:
