...
import h5py
from PGM.utils.utils_subtype import generate_split, nth
...
class Generic_WSI_MTL_Dataset(Dataset):
	...
	def test_split_gen(self, return_descriptor=False):
		if return_descriptor:
			...
			for task in range(len(self.label_dicts)):
				index = sorted(list(self.label_dicts[task].keys())) #[list(self.label_dicts[task].keys())[list(self.label_dicts[task].values()).index(i)] for i in range(self.num_classes[task])]
...
class Patient_SYBTYPE_MTL_Dataset(Generic_WSI_MTL_Dataset):
	def __init__(self,
		data_dir, 
		**kwargs):
		super(Patient_SYBTYPE_MTL_Dataset, self).__init__(**kwargs)
		self.data_dir = data_dir
		self.use_h5 = False
	...
	def __getitem__(self, idx):
		slide_id = self.slide_data['slide_id'][idx]
		sybtype = self.slide_data['label'][idx]
		if type(self.data_dir) == dict:
			source = self.slide_data['source'][idx]
			data_dir = self.data_dir[source]
		else:
		...
		if not self.use_h5:
			...
			return features, sybtype#label_list#site, sex
			...
			features = torch.from_numpy(features)
			return features, sybtype#site, sex, coords
        
        
class Generic_Split(Patient_SYBTYPE_MTL_Dataset):
	def __init__(self, slide_data, data_dir=None, num_classes=4, label_cols=None):
		self.use_h5 = False
		self.slide_data = slide_data
		self.data_dir = data_dir
